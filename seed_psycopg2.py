...
try:
    ...
    cur.execute("""
        INSERT INTO workshops (id, name) VALUES (999, 'Direct Workshop') ON CONFLICT (id) DO NOTHING;
    ...
    cur.execute("SELECT id, status, customer_view_token FROM jobs WHERE customer_view_token = 'GT3RSX';")
    print(cur.fetchall())
    
    cur.close()
    conn.close()
    print("Database seeded successfully via psycopg2.")
